...
public class metodosSueltos {
    ...
    public static int generarNumero(int min, int max) {
        ...
        if (rango <= 0L) {
            // Esto solo pasaría si el cálculo sobrepasa los límites de int (extremadamente
            // improbable aquí)
            throw new IllegalArgumentException("Rango inválido: demasiado grande");
        }
        // Math.random() devuelve [0.0,1.0). Multiplicando por rango obtenemos [0,
        // rango)
        long offset = (long) (Math.random() * rango); // offset dentro del rango
        return (int) (min + offset);
    }

    // Muestra diálogo de confirmación Si/No
    ...
    // Llena un combo con las marcas disponibles
    ...
    // Llena un combo con las categorías disponibles
